...
def create_datasets(data_path, input_size, rgb=False):
  ...
  train_data_tforms.append(transforms.RandomHorizontalFlip(0.5))
  train_data_tforms.append(transforms.RandomVerticalFlip(0.5))
...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    nn.Conv2d(in_channels=in_channels, out_channels=30, kernel_size=6,
        stride=1, padding=0, bias=False),
    nn.BatchNorm2d(30),
    nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
    nn.ReLU(),
    nn.Conv2d(in_channels=30, out_channels=10, kernel_size=4,
        stride=1, padding=0, bias=False),
    nn.BatchNorm2d(10),
    nn.Dropout(0.5),
    ...
    # self.features = nn.Sequential(
    #   nn.Conv2d(in_channels=in_channels, out_channels=10, kernel_size=6,
    #     stride=1, padding=0, bias=False),
    #   nn.BatchNorm2d(10),i
    #   nn.Dropout(0.5),
...
def custom_part1_trainer(model):
    ...
    # return  {'params':{'n_epochs': 200, 'batch_size': 50, 'experiment': 'part1'}}
    pass
    return None
